import { motion, useScroll, useTransform } from "framer-motion";
import { useRef } from "react";
import { Smartphone } from "lucide-react";

interface AppShowcaseProps {
  title: string;
  description: string;
  image: string;
  logo?: string;
  index: number;
}

export const AppShowcase = ({ title, description, image, logo, index }: AppShowcaseProps) => {
  const ref = useRef(null);
  const { scrollYProgress } = useScroll({
    target: ref,
    offset: ["start end", "end start"],
  });

  const scale = useTransform(scrollYProgress, [0, 0.5, 1], [0.8, 1, 1.2]);
  const opacity = useTransform(scrollYProgress, [0, 0.2, 0.8, 1], [0, 1, 1, 0.3]);

  return (
    <section ref={ref} className="relative h-screen w-full overflow-hidden snap-start snap-always">
      {/* Background Image with parallax */}
      <motion.div 
        className="absolute inset-0"
        style={{ scale }}
      >
        <div 
          className="absolute inset-0 bg-cover bg-center"
          style={{ backgroundImage: `url(${image})` }}
        />
      </motion.div>

      {/* Left gradient overlay */}
      <div className="absolute inset-0 bg-gradient-to-r from-black/90 via-black/60 to-transparent" style={{ width: '60%' }} />

      {/* Content */}
      <motion.div 
        className="relative h-full flex items-center px-6 md:px-12 lg:px-20"
        style={{ opacity }}
      >
        <div className="max-w-2xl space-y-8">
          {/* Title/Logo with stylized text - matching reference */}
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8, delay: index * 0.05 }}
            className="space-y-2"
          >
            {logo && (
              <img src={logo} alt={`${title} logo`} className="h-24 w-auto mb-4" />
            )}
            <h2
              className="text-6xl md:text-8xl font-black text-white tracking-tight leading-none"
              style={{
                textShadow: '3px 3px 6px rgba(0,0,0,0.6), 0 0 20px rgba(0,0,0,0.3)',
                WebkitTextStroke: '2px rgba(0,0,0,0.3)',
              }}
            >
              {title}
            </h2>
          </motion.div>

          {/* Description */}
          <motion.p
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8, delay: index * 0.05 + 0.15 }}
            className="text-xl md:text-2xl text-white leading-relaxed max-w-2xl font-light"
            style={{
              textShadow: '1px 1px 3px rgba(0,0,0,0.8)',
            }}
          >
            {description}
          </motion.p>

          {/* Download buttons */}
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8, delay: index * 0.05 + 0.3 }}
            className="flex flex-wrap gap-4 pt-2"
          >
            <a
              href="#"
              className="group relative overflow-hidden flex items-center gap-3 px-5 py-3.5 bg-black rounded-lg text-white transition-all duration-300 hover:scale-105 hover:shadow-2xl"
            >
              <svg viewBox="0 0 24 24" className="w-7 h-7" fill="currentColor">
                <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
              </svg>
              <div className="text-left">
                <div className="text-[10px] opacity-90 uppercase tracking-wide">Download on the</div>
                <div className="text-base font-semibold -mt-0.5">App Store</div>
              </div>
            </a>
            <a
              href="#"
              className="group relative overflow-hidden flex items-center gap-3 px-5 py-3.5 bg-black rounded-lg text-white transition-all duration-300 hover:scale-105 hover:shadow-2xl"
            >
              <svg viewBox="0 0 24 24" className="w-7 h-7" fill="currentColor">
                <path d="M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.6 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.5,12.92 20.16,13.19L17.89,14.5L15.39,12L17.89,9.5L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z"/>
              </svg>
              <div className="text-left">
                <div className="text-[10px] opacity-90 uppercase tracking-wide">Get it on</div>
                <div className="text-base font-semibold -mt-0.5">Google Play</div>
              </div>
            </a>
          </motion.div>
        </div>
      </motion.div>
    </section>
  );
};
